# 📡 Yufeed | Premium News Crawler & Dashboard

**Yufeed** adalah sistem *news crawler* modern yang dirancang untuk mengumpulkan, memproses, dan menyajikan berita dari berbagai sumber media besar di Indonesia secara *real-time*. Dilengkapi dengan dashboard admin berbasis **Shadcn UI** dan penyimpanan data **MySQL**.
...
- 🕵️ **Full Content Crawler**: Tidak hanya mengambil RSS, Yufeed dilengkapi mesin untuk menarik konten utuh dari halaman berita.
...
Buat database bernama `feedid` dan jalankan script migrasi (jika tersedia) atau gunakan `seed.js` untuk inisialisasi data awal:
```bash
node seed.js
```
...
Yufeed mendukung berbagai media besar Indonesia, antara lain:
...
---
**Yufeed** - *Gathering intelligence, one feed at a time.*
